            ...
            <tr key={index} className='border-b border-gray-500/20'>
              <td className='md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3'>
                ...
                <div className='flex-1'>
                  ...
                  <Line strokeWidth={2} percent={progress[index]?(progress[index].lectureCompleted*100) / progress[index].totalLecture : 0} className='bg-gray-300 rounded-full'/>
                </div>
              </td>
              ...
              <td className='px-4 py-3 max-sm:text-right'>
                <button onClick={()=>navigate('/player/'+course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 max-sm:text-xs text-white'>
                  {progress[index] && progress[index].lectureCompleted/progress[index].totalLecture===1?'Completed':'On Going'}
                </button>
              </td>
            </tr>
          ))}
